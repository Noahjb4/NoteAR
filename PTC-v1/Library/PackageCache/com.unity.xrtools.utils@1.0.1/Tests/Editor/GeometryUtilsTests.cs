...
namespace Unity.XRTools.Utils
{
    public class GeometryUtilsTests
    {
        ...
        static readonly float k_PointOnBoundsSmallEpsilon = 1E-6f;
        static readonly float k_PointOnBoundsLargeEpsilon = 1E-3f;
        ...
        static readonly List<Vector3> k_ExampleOcatagon = new List<Vector3>
        {
            new Vector3(1f, 0f, -1f),
        ...
        public void PointWithinHorizontalTriangle_PointOutsideTriangle(float x, float y, float z)
        {
            ...
            Assert.IsFalse(GeometryUtils.PointInHorizontalTriangle(point, k_ExampleTri[0], k_ExampleTri[1], k_ExampleTri[2]));
        }

        [TestCase(0.5f, 0f, 0.5f)]
        [TestCase(0.1f, 0f, 0.8f)]
        [TestCase(0f, 0f, 0.1f)]
        public void PointWithinHorizontalTriangle_PointInsideTriangle(float x, float y, float z)
        {
            var point = new Vector3(x, y, z);
            Assert.IsTrue(GeometryUtils.PointInHorizontalTriangle(point, k_ExampleTri[0], k_ExampleTri[1], k_ExampleTri[2]));
        }

        [Test]
        public void WithinEdgeZone_PointBetweenInnerEdgeAndOuterEdge()
        {
            var vector = Vector3.back * 0.7f + Vector3.right * 0.8f;
            Assert.IsTrue(GeometryUtils.WithinEdgeZone(vector, Vector3.zero, k_ExampleQuad, 0.5f));
        }

        [Test]
        public void WithinEdgeZone_PointInsideInnerEdge()
        {
            var vector = Vector3.back * 0.4f + Vector3.right * 0.3f;
            Assert.IsFalse(GeometryUtils.WithinEdgeZone(vector, Vector3.zero, k_ExampleQuad, 0.5f));
        }

        [Test]
        public void WithinEdgeZone_PointOutsideEdges()
        {
            var vector = Vector3.back * 4f + Vector3.right * 3f;
            Assert.IsFalse(GeometryUtils.WithinEdgeZone(vector, Vector3.zero, k_ExampleQuad, 0.5f));
        }
        ...
        [TestCaseSource(typeof(Polygon2DData), "SimpleConvex")]
        ...
        [TestCaseSource(typeof(Polygon2DData), "SimpleConvexAreas")]
        ...
        {
            Assert.True(GeometryUtils.PolygonInPolygon(k_ExampleTri, k_ExampleOcatagon));
        }
        ...
        {
            Assert.True(GeometryUtils.PolygonsWithinRange(k_ExampleTri, k_ExampleOcatagon, Mathf.Epsilon));
        }
        ...
        internal static class Polygon2DData
        {
            internal static List<Vector3>[] SimplePolygons = { k_ExampleTri, k_ExampleHexagon, k_ExampleHeptagon };
            static float[] SimplePolygonAreas = { 1f, 4f, 7f };
            ...
            public static IEnumerable SimpleConvexAreas
            {
                get
                {
                    ...
                    {
                        yield return new TestCaseData(SimplePolygons[i], SimplePolygonAreas[i]);
                    }
                }
            }
        }
