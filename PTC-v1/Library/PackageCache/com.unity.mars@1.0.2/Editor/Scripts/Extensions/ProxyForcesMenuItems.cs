...
namespace Unity.MARS.Forces.EditorExtensions
{
    ...
    static class ForcesMenuItemsRegions
    {
        ...
        static Material TryFindMaterial(string matName)
        {
            var paths = UnityEditor.AssetDatabase.FindAssets(matName);
            if ((paths == null) || (paths.Length < 1))
            {
                return null;
            }
            var astPath = UnityEditor.AssetDatabase.GUIDToAssetPath(paths[0]);
            var mat = UnityEditor.AssetDatabase.LoadAssetAtPath<Material>(astPath);

            return mat;
        }

        static void EditorTryApplyMaterial(GameObject gm, string matName)
        {
            var mat = TryFindMaterial(matName);
            if (mat)
            {
        ...
        static Transform EditorAddPlacementTransform(ref GameObject selection, string regionName, string matName)
        {
            if (!selection)
            {
            ...
            EditorTryApplyMaterial(newOb, matName);
        ...
        static ProxyRegionForceOccupancy AddOccupancyRegion(bool isEmpty, MenuCommand command)
        {
            ...
            var placement = EditorAddPlacementTransform( ref selection,
                ...
                isEmpty ? "ProxyForceRegion_Empty" : "ProxyForceRegion_Occupied" );
        ...
        public static void DrawForcesInInspectorForMainProxy(UnityObject targetObject)
        {
