...
{
    public class MARSPanel : EditorWindow, IHasCustomMenu
    {
...
#else
            typeof(PanelView).GetAssignableTypes(k_PanelTypes, type => !type.IsAbstract);
#endif
        ...
        {
            if (!(MARSUtils.CustomAddTabToHere(userData) is MARSPanel window))
                return;
