...
namespace Unity.MARS
{
    ...
    /// </summary>
    public class EntityVisualsModule : EditorScriptableSettings<EntityVisualsModule>, IModuleDependency<SlowTaskModule>
    {
        [Serializable]
...
#pragma warning restore 649
        ...
        void UpdateImageMarkerConditionVisuals()
        {
            ...
            foreach (var entityVisualMarkerContitionKeyValuePair in m_ImageMarkerConditions)
            {
                if (entityVisualMarkerContitionKeyValuePair.Key == null || entityVisualMarkerContitionKeyValuePair.Value == null)
                    continue;

                var foundMarkerId = false;
                MarkerCondition currentMarkerCondition = entityVisualMarkerContitionKeyValuePair.Value;
                for (var i = 0; i < loadedMarkerLibrary.Count; i++)
                {
                    if (loadedMarkerLibrary[i].MarkerId.ToString() == currentMarkerCondition.MarkerGuid)
                    {
                        entityVisualMarkerContitionKeyValuePair.Key.SetScale(new Vector3(loadedMarkerLibrary[i].Size.x, loadedMarkerLibrary[i].Size.y, 0.01f));
                        entityVisualMarkerContitionKeyValuePair.Key.SetTextureForImageMarker(loadedMarkerLibrary[i].Texture, MARSUserPreferences.instance.TintImageMarkers);
                        foundMarkerId = true;
                        break;
                    }
                }
                ...
                if (!foundMarkerId)
                {
                    ...
                    entityVisualMarkerContitionKeyValuePair.Key.SetTextureForImageMarker(Texture2D.whiteTexture, MARSUserPreferences.instance.TintImageMarkers);
                }
            }
        }
