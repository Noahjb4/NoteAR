...
namespace Unity.MARS.Query
{
        ...
        RelationRatingsData[]>
    {
        ...
        public static bool RateMatches<TChild1Values, TChild2Values>(
            IRelationRatingMethod<TChild1Values, TChild2Values> relation,
            ref RelationInput<TChild1Values, TChild2Values> inputValues,
            ref NativeConditionRatings ratingStorage,
            ref int relationIndex)
            where TChild1Values : struct, IRelationChildValues
            where TChild2Values : struct, IRelationChildValues
        {
            var count = 0;
            ratingStorage.StartCondition(relationIndex);

            for (var i = 0; i < inputValues.Count; i++)
            {
                var child1Value = inputValues.Child1Values[i];
                var child2Value = inputValues.Child2Values[i];
                var rating = relation.RateDataMatch(ref child1Value, ref child2Value);
